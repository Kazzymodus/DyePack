...
namespace Kazzymod.Projectiles.Minions
{
    class MortarHydra : HydraHead
    {
        ...
        protected override int AttackCooldown { get; } = 200;
        ...
        protected override void Attack()
        {
            ...
            if (direction.Y > 0) // Target is below hydra.
            {
                ...
                float arcTime = 2 * (yVelocity / Gravity);

                float a = Gravity;
                float b = 2 * yVelocity - Gravity;
                float c = -2 * direction.Y;

                float fallTime = Quadratic(a, b, c);
                float totalTime = arcTime + fallTime;
            ...
            else
            {
                ...
                float crestDistance = direction.Y - CrestHeight;
                float crestTime = Quadratic(1, 1, crestDistance * 2 / Gravity);
                ...
                float fallTime = Quadratic(1, 1, -CrestHeight * 2 / Gravity);

                float totalTime = crestTime + fallTime;
                direction.X += totalTime * target.velocity.X;
                xVelocity = direction.X / totalTime;
            }
